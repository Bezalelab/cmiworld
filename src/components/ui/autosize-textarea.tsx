  ...
  ({ maxHeight = Number.MAX_SAFE_INTEGER, minHeight = 40, className, onChange, value, ...props }: AutosizeTextAreaProps, ref: React.Ref<AutosizeTextAreaRef>) => {
    ...
    return (
      <textarea
        ...
        value={value}
        ref={textAreaRef}
        className={cn('peer block rounded-none w-full appearance-none h-10 overflow-hidden resize-none border-b border-gray-2 bg-transparent px-0 py-2.5 text-sm text-black focus:outline-none focus:ring-0', className)}
        onChange={(e) => {
