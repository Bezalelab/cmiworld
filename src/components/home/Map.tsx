...
export function Map({ countriesData }) {
  ...
  const handleStateChange = (newState) => {
    const newIndex = items.findIndex((item) => item.title === newState);

    if (window.innerWidth > 768) {
      gsap.to(progressBarRef.current, {
        height: `${(newIndex / (items.length - 1)) * maxBarHeight}px`,
        width: '3px',
        duration: 0.5,
        ease: 'power2.inOut',
      });
            ...
            <ol className="relative flex justify-between lg:z-0 lg:flex-col lg:gap-10 lg:pl-14">
              {items.map((item, index) => (
                <li
                  ...
                  onClick={() => handleStateChange(item.title)}>
